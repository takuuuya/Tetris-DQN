...
ENV_NAME = 'Tetris'  # Environment name
#FRAME_WIDTH = 16  # Resized frame width
#FRAME_HEIGHT = 22  # Resized frame height
#FRAME_WIDTH = (BOXSIZE*(BOARDWIDTH+2)+100)/20  # Resized frame width
#FRAME_HEIGHT = (BOXSIZE*(BOARDHEIGHT+2))/20  # Resized frame height
...
SAVE_NETWORK_PATH = 'saved_networks/' + ENV_NAME + '_{0}x{1}_episodes:{2}'.format(BOARDWIDTH, BOARDHEIGHT, NUM_EPISODES)
SAVE_SUMMARY_PATH = 'summary/' + ENV_NAME + '_{0}x{1}_episodes:{2}'.format(BOARDWIDTH, BOARDHEIGHT, NUM_EPISODES)
...
class Agent():
    ...
    def run(self, state, action, reward, terminal, observation):
        ...
        if (reward != 0):
            print('reward = {0} / total_reward = {1}'.format(reward, self.total_reward))
    ...
    if TRAIN:  # Train mode
        ...
        for _ in range(NUM_EPISODES):
            ...
            for _ in range(random.randint(1, NO_OP_STEPS)):
                ...
                env.execute_action(5, player_count)  # Do nothing
                observation, _, _ = env.observe()
            state = agent.get_initial_state(observation, last_observation)
            while not terminal:
                last_observation = observation
                if ( PLAYMODE and player_count <= PLAYGAME ):
                    action = agent.play_action()
                    pygame.event.clear()
                else:
                    action = agent.get_action(state)
                env.execute_action(action, player_count)  # Do nothing
        ...
        for _ in range(NUM_EPISODES_AT_TEST):
            ...
            for _ in range(random.randint(1, NO_OP_STEPS)):
                last_observation = observation
                env.execute_action(5)  # Do nothing
                observation, _, _= env.observe()
            state = agent.get_initial_state(observation, last_observation)
            while not terminal:
                last_observation = observation
                action = agent.get_action(state)
                env.execute_action(action)  # Do nothing
                observation, reward, terminal = env.observe()
                #env.render()
                processed_observation = preprocess(observation, last_observation)
